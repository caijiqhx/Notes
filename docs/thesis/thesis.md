...
> 以上部分已修改，具体见 latex 文件

## 算法设计与实现

上文已分别介绍了程序作者身份归属技术和代码混淆算法的分类与评估，本章将分析程序作者身份归属中依赖的二进制文件特征，设计相应的代码混淆算法以降低识别准确率。

### 程序归属

早期的程序作者归属研究仅针对程序源代码文件，依赖于诸如缩进和变量命名等文本特征以及抽象语法树等语法特征，实现了较高的成功率。而在二进制程序的作者归属中，通常无法获取程序源代码，而源代码的文本特征在编译过程中被删除。现有的基于机器学习的作者归属技术 [1-4] 提取大量的二进制程序静态特征，经过监督学习筛选出能够反映程序作者风格的特征，作为后续程序分类的基础。研究表明，以汇编指令序列为代表的局部静态特征与以控制流图为代表的全局静态特征对程序作者归属结果有较大影响 [1]。而这些静态特征往往易被修改因此本文将设计对此类静态特征进行混淆变换而干扰程序作者归属技术的结果。
